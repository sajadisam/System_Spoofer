...
Registry::Registry(const std::string &path)
        : m_Path(path), m_Loaded(false), m_LoadedKeys(false), m_LoadedValues(false) {
...
void Registry::Open() {
    ...
    if (openResult != ERROR_SUCCESS) {
        switch (openResult) {
            ...
            default:
                throw std::runtime_error("Failed opening key: " + m_Path +
...
bool Registry::SetKeyName(const std::string &name) noexcept {
    if (!Available())
        return false;
...
void Registry::TakeOwnership() {
    ...
    try {
        ...
        dwRes = SetNamedSecurityInfo(
                str, SE_REGISTRY_KEY, OWNER_SECURITY_INFORMATION, pSIDAdmin, NULL, NULL, NULL);
        ...
        dwRes = SetNamedSecurityInfo(
                str, SE_REGISTRY_KEY, DACL_SECURITY_INFORMATION, NULL, NULL, pACL, NULL);
